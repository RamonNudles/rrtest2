...
RunService.RenderStepped:Connect(function()
    ...
    -- aim above the head
    local topOfHead = head.Position + Vector3.new(0, head.Size.Y/2 + ExtraVertical, 0)
    local screenPos, onScreen = Camera:WorldToViewportPoint(topOfHead)
	...
	-- Aim lock on right-click (skips dead)
	RunService.RenderStepped:Connect(function()
		if RightDown then
			local head = getClosestPart()
			if head then
				-- use head.Position directly so lock stays centered even when above
